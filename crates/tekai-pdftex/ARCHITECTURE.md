...
`crates/tekai-engine` is Tekai's self-contained exact engine. It is valuable as a
...
`crates/tekai-pdftex/src/native.rs` is the fast native subset. It already
...
The target engine keeps the fidelity discipline of `tekai-engine` and the
...
another compatibility layer. The fallback remains `tekai-engine` or external
...
The new engine should live under `crates/tekai-pdftex` as a v2 core rather
than inside generated `crates/tekai-engine`.

Suggested module layout:

```text
crates/tekai-pdftex/src/
  engine/
    mod.rs
    dist.rs
    snapshot.rs
...
`src/compiler.rs` should eventually dispatch `Engine::TekaiPdftex` directly
