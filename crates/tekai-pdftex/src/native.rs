...
pub fn run_native_with_artifact_policy(
    ...
    let trace_path = options
        .output_dir
        .join(format!("{}.tekai-pdftex.trace", options.job_name));
    ...
    trace.push(TraceEvent::new("engine", "tekai-pdftex-native"));
...
fn write_success_log(
    ...
    writeln!(
        file,
        "This is tekai-pdftex experimental native engine, compatibility level 0."
    )?;
...
mod tests {
    ...
    fn temp_dir(name: &str) -> PathBuf {
        let root = std::env::temp_dir().join(format!("tekai-pdftex-{name}-{}", std::process::id()));
...
",
        ...
        assert!(out.join("main.tekai-pdftex.trace").exists());
    }
...
",
        ...
        assert_eq!(run.status, NativeEngineStatus::Native);
        let trace = fs::read_to_string(out.join("main.tekai-pdftex.trace")).expect("trace exists");
        assert!(
...
\hypersetup{pdfsubject={Native Subject}, pdfkeywords={alpha, beta}, pdfcreator={tekai}}
...
",
        ...
        assert!(pdf_text.contains("/Creator (tekai)"), "{pdf_text}");
...
",
        ...
        let trace = fs::read_to_string(out.join("nativejob.tekai-pdftex.trace"))
