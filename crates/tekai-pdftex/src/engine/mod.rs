...
mod tests {
    ...
    fn report_only_runner_discovers_inputs_and_emits_ledger() {
        let root = unique_temp_dir("tekai-pdftex-engine-report");
...
"#,
    ...
    fn report_only_runner_names_unsupported_effects() {
        let root = unique_temp_dir("tekai-pdftex-engine-unsupported");
        fs::write(
            root.join("main.tex"),
            r#"\documentclass{article}
\begin{document}
...
"#,
    ...
    fn report_only_runner_handles_bundled_arxiv_examples() {
        ...
        for relative in [
            ...
            let report = run_report_only(&EngineV2Options {
                main: main.clone(),
                output_dir: workspace.join("target/tekai-pdftex-report-only"),
                job_name: "main".to_string(),
            })
