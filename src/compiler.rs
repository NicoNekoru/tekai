...
const BUILD_STATE_VERSION: u32 = 33;
...
pub fn build(options: &BuildOptions) -> Result<BuildReport> {
    match options.runner {
        Runner::Direct if options.engine == Engine::TexpilotPdftex => {
...
fn latexmk_or_tectonic_build(options: &BuildOptions) -> Result<BuildReport> {
    ...
    let mut command = match options.engine {
        ...
        Engine::TexpilotPdftex => unreachable!("texpilot-pdftex was rejected above"),
    };
...
fn nonfinal_output_mode_arg(engine: Engine) -> Option<&'static str> {
    match engine {
        Engine::PdfLatex | Engine::LuaLatex | Engine::TexpilotPdftex => Some("-draftmode"),
...
fn engine_program(engine: Engine) -> &'static str {
    match engine {
        ...
        Engine::TexpilotPdftex => "pdflatex",
    }
}

fn uses_pdftex_graphics_pipeline(engine: Engine) -> bool {
    matches!(engine, Engine::PdfLatex | Engine::TexpilotPdftex)
}
...
fn latexmk_command(
    ...
    match options.engine {
        ...
        Engine::TexpilotPdftex => unreachable!("texpilot-pdftex does not use latexmk"),
    }
    command
        .arg("-interaction=nonstopmode")
        .arg("-halt-on-error")
