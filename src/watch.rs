...
const HOT_PREVIEW_DIR: &str = ".texpilot-hmr";
const HOT_PREVIEW_WARM_DIR: &str = ".texpilot-hmr-warm";
...
impl HotPreviewState {
    ...
    fn prepare_document(
        ...
        build_options.engine = Engine::TexpilotPdftex;
...
fn assemble_hot_preview_source(preamble: &str, snippet: &str, target_path: &Path) -> String {
    format!(
        "% Generated by texpilot watch --preview for {}.\n{}\\begin{{document}}\n\\pagestyle{{empty}}\n{}\n\\end{{document}}\n",
        target_path.display(),
        preamble,
        snippet
    )
...
mod tests {
    ...
    fn hot_preview_preamble_strips_metadata_and_inlines_macro_inputs() {
        let root = unique_temp_dir("texpilot-hot-preview-preamble");
    ...
    fn hot_preview_moves_definition_only_document_inputs_to_preamble() {
        let root = unique_temp_dir("texpilot-hot-preview-definitions");
        fs::create_dir_all(root.join("content")).expect("failed to create content directory");
        fs::write(
    ...
    fn hot_preview_snippet_rejects_root_preamble_edits() {
        let root = unique_temp_dir("texpilot-hot-preview-root-edit");
        let main = root.join("main.tex");
        let source = "\\documentclass{article}\n\
            \\newcommand{\\x}{1}\n\
            \\begin{document}\n\
    ...
    fn hot_preview_snippet_trims_an_unmatched_environment_tail() {
        let root = unique_temp_dir("texpilot-hot-preview-environment-tail");
        let main = root.join("main.tex");
    ...
    fn hot_preview_rejects_mixed_or_structural_change_sets() {
        let root = unique_temp_dir("texpilot-hot-preview-structural-change");
        fs::create_dir_all(&root).expect("failed to create temp directory");
        let main = root.join("main.tex");
    ...
    fn output_dir_canonicalizes_an_existing_symlinked_parent() {
        let root = unique_temp_dir("texpilot-watch-output-symlink");
