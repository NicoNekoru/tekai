...
#[test]
fn texpilot_pdftex_native_backend_builds_minimal_document() {
    let root = unique_temp_dir("texpilot-pdftex-native");
...
#[test]
fn texpilot_pdftex_native_backend_writes_table_of_contents() {
    let root = unique_temp_dir("texpilot-pdftex-native-toc");
    fs::create_dir_all(&root).expect("failed to create temp directory");
    let main = root.join("main.tex");
    let out_dir = root.join("out");
...
#[test]
fn texpilot_pdftex_native_backend_writes_float_lists() {
    let root = unique_temp_dir("texpilot-pdftex-native-float-lists");
    fs::create_dir_all(&root).expect("failed to create temp directory");
    let main = root.join("main.tex");
    let out_dir = root.join("out");
...
#[test]
fn texpilot_pdftex_native_backend_writes_index_sidecar() {
    let root = unique_temp_dir("texpilot-pdftex-native-index");
    fs::create_dir_all(&root).expect("failed to create temp directory");
    let main = root.join("main.tex");
    let out_dir = root.join("out");
...
#[test]
fn texpilot_pdftex_native_backend_writes_pdf_metadata() {
    let root = unique_temp_dir("texpilot-pdftex-native-pdf-metadata");
    fs::create_dir_all(&root).expect("failed to create temp directory");
    let main = root.join("main.tex");
    let out_dir = root.join("out");
...
#[test]
fn texpilot_pdftex_native_backend_writes_hyperref_out() {
    let root = unique_temp_dir("texpilot-pdftex-native-hyperref-out");
    fs::create_dir_all(&root).expect("failed to create temp directory");
    let main = root.join("main.tex");
...
#[test]
fn texpilot_pdftex_native_auto_mode_stays_single_pass_without_external_bibtex() {
    let root = unique_temp_dir("texpilot-pdftex-native-single-pass");
    fs::create_dir_all(&root).expect("failed to create temp directory");
    let main = root.join("main.tex");
    let refs = root.join("refs.bib");
    let out_dir = root.join("out");
...
#[test]
fn texpilot_pdftex_falls_back_to_real_pdflatex_for_unsupported_documents() {
    if !command_available("pdflatex") {
        eprintln!("skipping texpilot-pdftex fallback test; pdflatex is not available");
        return;
    }

    let root = unique_temp_dir("texpilot-pdftex-fallback");
    fs::create_dir_all(&root).expect("failed to create temp directory");
    let main = root.join("main.tex");
    let out_dir = root.join("out");
    fs::write(&main, FALLBACK_DOC).expect("failed to write TeX source");

    let report = build(&options(&main, &out_dir)).expect("fallback texpilot-pdftex build failed");
...
#[test]
fn texpilot_pdftex_native_backend_writes_synctex_when_requested() {
    let root = unique_temp_dir("texpilot-pdftex-native-synctex");
    fs::create_dir_all(&root).expect("failed to create temp directory");
    let main = root.join("main.tex");
    let out_dir = root.join("out");
    fs::write(&main, NATIVE_DOC).expect("failed to write TeX source");
...
#[test]
fn texpilot_pdftex_fallback_preserves_pdflatex_bibtex_scheduler() {
    if !command_available("pdflatex") || !command_available("bibtex") {
        eprintln!(
            "skipping texpilot-pdftex fallback BibTeX test; pdflatex or bibtex is unavailable"
        );
        return;
    }

    let root = unique_temp_dir("texpilot-pdftex-fallback-bibtex");
    fs::create_dir_all(&root).expect("failed to create temp directory");
    let main = root.join("main.tex");
    let refs = root.join("refs.bib");
    let out_dir = root.join("out");
